module.exports.User = require("./user.model");
module.exports.Permission = require("./permission.model");
module.exports.MenuItem = require("./menuItem.model");
module.exports.Order = require("./order.model");
module.exports.Table = require("./table.model");
module.exports.Inventory = require("./inventory.model");
module.exports.Expense = require("./expense.model");
module.exports.Income = require("./income.model");
module.exports.Category = require("./category.model");
module.exports.Supplier = require("./supplier.model");
module.exports.SupplierLog = require("./supplierLog.model");
module.exports.OrderInventoryLog = require("./orderInventoryLog.model");
module.exports.CashOpening = require("./cashOpening.model");
module.exports.BankDeposit = require("./bankDeposit.model");
module.exports.Wastage = require("./wastage.model");
module.exports.KitchenInventory = require("./kitchenInventory.model");
module.exports.Buyer = require("./buyer.model");
module.exports.Oil = require("./oil.model");
module.exports.EmployeeProfile = require("./employeeProfile.model");
module.exports.Attendance = require("./attendance.model");
module.exports.SalaryRecord = require("./salaryRecord.model");
